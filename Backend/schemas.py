...
class CreateUserRequest(BaseModel):
    ...
    password: str = Field(
        min_length=5, max_length=35, description="Password field of the user"
    )
    userPlaylists: str = Field(description="User playlists")
    userProfilePhoto: Optional[str] = Field(
        None, description="URL or ID of user profile picture"
    )
